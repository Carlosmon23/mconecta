...
const ArtistPage: React.FC<ArtistPageProps> = ({ artist, onBook, currentUser, onToggleFollow }) => {
    ...
    const handleBooking = () => {
        ...
        setTimeout(() => {
            ...
            onBook({
                type: 'event', // This should be dynamic based on user input, simplified for now
                date: selectedDate,
                eventType: 'Evento Geral'
            });
            // Reset after short delay
            setTimeout(() => setBookingStep('idle'), 2000);
        }, 1000);
    };
        ...
        <div className="pt-24 pb-20 animate-fadeIn">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-2 space-y-10">
                    ...
                    <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                className="p-2 hover:bg-gray-100 rounded-full transition-colors flex flex-col items-center text-xs gap-1 group">
                                <Heart size={20} className={currentUser?.followedArtistIds?.includes(artist.id) ? "fill-brand-primary text-brand-primary" : "text-gray-700 group-hover:text-brand-primary"} />
                                <span className="underline opacity-0 group-hover:opacity-100 transition-opacity">
                                    {currentUser?.followedArtistIds?.includes(artist.id) ? 'Seguindo' : 'Seguir'}
                                </span>
                            </button>
                        </div>
                    </div>
                    ...
                    {/* Professional Details Chips */}
                    <div className="flex flex-wrap gap-2">
                        {artist.formation && (
                            <div className="px-3 py-1 bg-gray-100 rounded-full text-xs font-bold text-gray-700 uppercase tracking-wide">
                                Formação: {artist.formation}
                            </div>
                        )}
                        {artist.availability && (
                            <div className="px-3 py-1 bg-green-50 rounded-full text-xs font-bold text-green-700 uppercase tracking-wide">
                                Disponibilidade: {artist.availability}
                            </div>
                        )}
                        {artist.eventTypes && artist.eventTypes.map(t => (
                            <div key={t} className="px-3 py-1 bg-blue-50 rounded-full text-xs font-bold text-blue-700 uppercase tracking-wide">
                                {t}
                            </div>
                        ))}
                        {artist.projectTypes && artist.projectTypes.map(t => (
                            <div key={t} className="px-3 py-1 bg-purple-50 rounded-full text-xs font-bold text-purple-700 uppercase tracking-wide">
                                Projeto: {t}
                            </div>
                        ))}
                    </div>
